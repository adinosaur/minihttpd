...
#ifndef MINIHTTPD_MD5_H
#define MINIHTTPD_MD5_H
...
// 最后加上64位的原来长度
//
std::shared_ptr<std::vector<unsigned int>> add(string str)
{
    // 以512位,64个字节为一组
    ...
    std::shared_ptr<std::vector<unsigned int>> strByte = std::make_shared<vector<unsigned int>>(num * 16);
    strlength = num * 16;
